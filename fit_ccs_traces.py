...
def fit_ccs_traces_page():
    ...
    # Create a list to store Gaussian fits for saving later
    gaussian_fit_params = []

    # If summed, create a common x-grid
    ...
    # Save CSV with Gaussian fit parameters
    gaussian_params_df = pd.DataFrame(gaussian_fit_params, columns=[f"Mean_{i+1}" for i in range(num_peaks)] +
                                              [f"StdDev_{i+1}" for i in range(num_peaks)] +
